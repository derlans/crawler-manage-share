import {
  favoriteCommunityApi,
  likeCommunityApi,
  unfavoriteCommunityApi,
  unlikeCommunityApi,
} from '@/api/community';
...
export const columns = [
  ...
  },
  {
    ...
    render(row) {
      const onClick = async () => {
        if (row.isLike) {
          await unlikeCommunityApi({ _id: row._id });
          row.likeCount--;
        } else {
          await likeCommunityApi({ _id: row._id });
          row.likeCount++;
        }
  ...
  },
  {
    ...
    render(row) {
      const onClick = async () => {
        if (row.isFavorite) {
          await unfavoriteCommunityApi({ _id: row._id });
          row.favoriteCount--;
        } else {
          await favoriteCommunityApi({ _id: row._id });
          row.favoriteCount++;
        }
